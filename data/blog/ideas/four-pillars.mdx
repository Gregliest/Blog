...
I’m going to caveat this post by acknowledging that the field is moving very fast. I’ve been thinking about the 4 Pillars for over three years now and much has changed in the last 6 months. I’ve had to update this post as I was writing it with new advances, and I’m certainly not aware of everything that is happening.
...
A schema is a simplified representation of some part of the world, and our brains have learned schemas for everything. For instance, when you throw a ball, your brain has a physics model that predicts where the ball is going to go. When you interact with someone, your brain is constantly trying to figure out what they are thinking, and what they are going to do next. These models represent the sum of our experience, distilled as much as possible. With these models, we can do higher level reasoning, like prediction and induction. We can infer causality, and we can predict things that we've never seen before. While we build most of these models through experience, Dr. Dehaene presents some fascinating evidence that we are born with some of our models. Innate models might have some interesting applications to theories like the [Lottery Ticket Hypothesis](https://arxiv.org/abs/1803.03635).

I believe that schemas are the key to the next generation of AI. I’m going to use “schema” instead of “model” because model has all sorts of other connotations within machine learning. Schemas can be built manually or automatically. For instance, self driving car algorithms are highly modular. The engineers have broken down the problem into smaller chunks, and each chunk is solved independently. Essentially, they have manually dictated or discovered some structure to the problem, by identifying intermediate problems to be solved, and that structure represents a sort of schema for self-driving cars. However, obviously it would be more flexible and powerful if AI could generate these schemas automatically. Once AI can automatically generate schemas, we can finally move away from AI as only statistical models, and I think we will quickly cross the threshold to a general intelligence.

Automated schemas in AI are not well solved. The field of model-based reinforcement learning has stalled for the last 5 or more years. There’s some interesting work going on in pruning and sparsity, but that field is pretty nascent as far as I know. Theoretically, a pruned network might be able to discover structure, and thus do abstraction. While I was writing this post, some really interesting new tools came out like [ViperGPT](https://github.com/cvlab-columbia/viper) and [Segment Anything](https://segment-anything.com/), which seem to have notions of correctness and abstraction, but are limited to images. There are also many more tools for generating code from prompts. These tools could be the first step towards building automated backing schemas.
...
Curiosity is the ability to figure out what you don’t know, and gather more information about it. It requires that you can take actions to find the knowlege you lack. Curiosity is key to how humans learn: just hang out with any kid, and wait for the “why …?” The child has identified that they don’t know something, and then taken an action (asking the question) to find out more.
