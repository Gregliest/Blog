...
B) Gary is now the Bey of Bowling, and throws a strike every time.
...
A posterior of .001 is not at all convincing: we’re going to need a lot more evidence to be convinced. So, let’s gather some more data. Gary proceeds to throw another 7 strikes, for a total of 10 in a row!

$\frac{P(B)}{P(A)} = 10^{-6}$\
$P(D | A) = (1/10)^10 = 10^{-10}$ -> The data is extraordinarily unlikely, given hypothesis A.\
$P(D | B) = 1*1*1 = 1$
...
$Posterior for B vs A= 10^{-6} * \frac{.035}{9e-11} = 387$
...
$Posterior for B vs C = 10^{-6} * \frac{.035}{1} = 3.5e-8$
...
$Posterior for B vs C= .1 * \frac{.0012}{1} = 1.2e-9$
